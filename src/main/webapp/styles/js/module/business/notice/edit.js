...
requirejs(['jquery', , 'bootstrap', 'fuelux', 'switchs', 'select', 'selectCN', 'validator', 'vb', 'validatorLAG', 'comm', 'form', 'message'],
    function ($, _) {
        ...
        if (notice != undefined && notice != null && notice != "") {
            //初始化页面
            $("#habitId").val(notice.habit_id);
            $("#content").val(notice.name);
            $("#picture").val(notice.url);
            meForm($('#formSubmit'), notice);
        }
        else {
        ...
        ///habit/notice/list/
        var listAction =  '/habit/notice/list/'+$("#habitId").val();
        ...
        $('#formSubmit').formValidation({
            ...
            if (($("#picture").val() == '' )&&( $("#file").val() == '')) {
                ...
                }else
                {
                    ...
                    $.post(WEB_GLOBAL_CTX + "/habit/notice/update", params, function (rsp) {
                        if (rsp.successful) {
                            $.scojs_message(rsp.msg, $OK);
                            $("#save").toggleClass("disabled");
                            setTimeout("window.location.href='" + listAction+ "'", 1000);
